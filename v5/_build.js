...
AndroidBuilder.prototype.validate = function validate(logger, config, cli) {
	...
	return function (callback) {
		// called for every module in the app (not the Ti provided ones)
		this.validateTiModules('android', this.deployType, function (err, modules) {
			this.modules = modules.found;

			this.commonJsModules = [];
			this.nativeLibModules = []; // Ti provided native platform modules
...
AndroidBuilder.prototype.run = function run(logger, config, cli, finished) {
	...
	appc.async.series(this, [
		...
		function (next) {
			cli.emit('build.pre.compile', this, next); // Will trigger AAR extraction to build/intermediates/exploded-aar
		},

		'createBuildDirs',
		'copyResources',    	// app resources only NOTE does register module resources regardless name!?!
		'generateRequireIndex',
		'processTiSymbols', 	// runs aapt (aar based resource have not to be copied!)
		'copyModuleResources',  // module (but not aar) resources TODO sg resources from modules? too late for aapt?
		'removeOldFiles',
		'generateJavaFiles',
		'generateAidl',
...
AndroidBuilder.prototype.initialize = function initialize(next) {
    ...
    this.aarResPackages = {}, // key = path, value = package
    this.aarClassPaths = {}; // key = path to jar, value = 1
...
AndroidBuilder.prototype.copyResources = function copyResources(next) {
	var ignoreDirs = this.ignoreDirs,
		...
		drawableResources = {},
		jsFiles = {},
		moduleResPackages = this.moduleResPackages = [],  // ResPackage of module itself
		aarLibResPackages = {},
	...
	this.modules.forEach(function (module) {
		...
		if (fs.existsSync(respackagepath)) {
            ...
            this.moduleResPackages.push(data[0]);  // was shift in original code
		}
	}, this);
...
AndroidBuilder.prototype.copyModuleResources = function copyModuleResources(next) {
	...
	var tasks = Object.keys(this.jarLibraries).map(function (jarFile) {
			return function (done) {
				...
				if (fs.existsSync(resPkgFile) && fs.existsSync(resFile)) {
					// track respackage for later aapt processing
	...
	this.nativeLibModules.forEach(function (m) {
        var src = path.join(m.modulePath, 'platform','android','assets');
		this.logger.debug("Native Module Assets check Dir " + src);
		if (fs.existsSync(src)) {
			this.logger.debug("Found - copy content");
			tasks.push(function (done) {
                copy(src, this.buildBinAssetsDir);
				done();
			}.bind(this));
		}
	}, this);
...
AndroidBuilder.prototype.generateAndroidManifest = function generateAndroidManifest(next) {
    ...
    // add the analytics service, if configured in TIApp.xml
...
AndroidBuilder.prototype.packageApp = function packageApp(next) {
	...
	// we can start aapt directly, when no respackage to handle
    ...
    Object.keys(this.aarResPackages).forEach(function (resDir) {
        namespaces && (namespaces+=':');
        namespaces += this.aarResPackages[resDir];
    }, this);
...
AndroidBuilder.prototype.compileJavaClasses = function compileJavaClasses(next) {
	var classpath = {},
		...
		jarNames = {},
		aarClassPaths = this.aarClassPaths;
    ...
    var l =  Object.keys(aarClassPaths);
    var aarLen = Object.keys(aarClassPaths).length;
	for(var i=0;i<aarLen;i++) {
		var currentJarPath = l[i];
        var jarHash = this.hash(fs.readFileSync(currentJarPath).toString());
        if (!jarNames[jarHash]) {
            moduleJars[currentJarPath] = 1;
            classpath[currentJarPath] = 1;
            jarNames[jarHash] = 1;
        } else {
    ...
    this.modules.forEach(function (module) {
		if (fs.existsSync(module.jarFile)) {
			var jarHash = this.hash(fs.readFileSync(module.jarFile).toString()); // Hash of content

			if (!jarNames[jarHash]) {
				moduleJars[module.jarFile] = 1;  // will go into dexer args
